...
(define poly (make-sparse-poly 'x 
	(list (list 5 (make-scheme-number 1)) 
				(list 0 (make-scheme-number -1)))))
(display poly) 

(define poly2 (make-sparse-poly 'x 
	(list (list 2 (make-scheme-number 1))
				(list 0 (make-scheme-number -1)))))
(display poly2)

(display (div poly poly2))


;(polynomial (sparse-poly x (3 (scheme-number . 1)) 
;													 (1 (scheme-number . -1))) 
;((1 (scheme-number . 1)) (0 (scheme-number . -1))) )
